...
const IdentityObject = ({address, issuer, txid, timestamp, chain, testnet, isRevoked, revokedAt, expireDate, state}) => {
  return (
...
const ErrorMsg = ({customText, docType}) => (
    <div className="drop-shadow-gray border border-gray-50 rounded-md p-5">
      <ul className={'text-left'}>
        ...
        <li className={'grid grid-cols-9'}>
          <FontAwesomeIcon icon={faInfoCircle} className="text-primary mt-1"/>
          <div className="col-span-8 mx-1">Хувилбар болон засварласан файл хүчингүйд тооцогдоно.</div>
        </li>
      </ul>
        ...
        {error && (
            <>
              <div id="result_invalid" className="bc-alert bc-alert-danger bc-text-center">
                <div className={'text-[#DB9390] text-xl font-medium'} style={{color: '#DB9390',}}>{error.detail}</div>
              </div>
              <div className={'msg'}>
            ...
            <>
              {result.state === 'ISSUED' && (
                  <div className={'grid grid-cols-4 gap-2'}>
                    <div className={'bg-white drop-shadow-gray rounded-2xl flex justify-center items-center'}>
                      <img src={certified} className={'h-12'} alt='valid'/>
                    </div>
                    <div id="result_valid"
                         className={classNames('col-span-3 text-center bg-secondary-yellow text-primary-yellow uppercase font-medium text-xl drop-shadow-yellow state py-9 rounded-2xl')}
                    >
                      Хүчинтэй
                    </div>
                  </div>)}
              {result.state === 'EXPIRED' && (
                  <div id="result_revoked_expired" className="bc-alert bc-alert-danger bc-text-center">
                    <div className={'text-[#DB9390] text-xl font-medium'} style={{color: '#DB9390',}}>Хугацаа дууссан</div>
                  </div>
              )}
              {result.state === 'REVOKED' && (
                  <div id="result_revoked_expired" className="bc-alert bc-alert-danger bc-text-center">
                    <div className={'text-[#DB9390] text-xl font-medium'} style={{color: '#DB9390',}}>Хүчингүй</div>
                  </div>
              )}
